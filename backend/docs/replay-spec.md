...
- **`POST /v1/attachments/{id}/locate` 体 `exclude`：按下「都不是」就往更早的历史里找**（2026-09-11 补充）。人说「都不是」的意思是「这段历史里没有」，不是「再算一遍」：阈值没变、窗口没变，在同一批永久索引上重跑必然逐字节吐回同样三条（线上 `b01394f2` 那张图连按五次「都不是」，排出五条 job，三段候选完全相同）。所以带了 `exclude` 的**手动**定位改的是搜索范围：`ensure_index` 沿时间轴分段往前推，一段 `SPAN = 3 × LOCATE_WINDOWS[2] = 768` 根（与重温窗口同宽），第 n 段是 `[T0−(n+1)×SPAN, T0−n×SPAN]`。第 0 段与从前逐字节相同——复盘发布后的自动定位、以及第一次手动定位，一格没动。n≥1 的段结尾往后放宽 `LOCATE_WINDOWS[2] − 1 = 255` 根：一个 256 根的窗口要是正好横跨两段交界，两段都不完整包含它，不放宽就会在交界处留一个谁也没建过的洞；多算的那一截是白算，`public_market.features` 的 id 是坐标与输入的 md5，重复行 `ON CONFLICT DO NOTHING` 落不进去。推到第几段由后端自己数（从第 0 段往上找第一段没建过的），不由客户端传——客户端算错一轮，人就白点一次。上限 `MAX_SPANS = 8` 段（1h 上约八个半月）；推到顶，或者 `index_range` 只拉回上市之后那一截（凭 `actual_start` 晚于请求起点认）、或者一根都没拉到，结果里记 `exhausted:true`、`reason` 为 `range_exhausted`，能建的仍旧建进去。`exclude` 是累加的一份 id（第三轮要把前两轮一共六条都带上），先去重再看上限（`MAX_EXCLUDE` 100 条，越界 422 `locate_exclude_too_many`），不给的时候请求体、指纹与从前完全一致。排除本身怎么生效（写在 ANN 取数那一层、在 `LIMIT 3000` 之前）见 `docs/history-search.md`。
