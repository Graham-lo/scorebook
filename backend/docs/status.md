...
> 2026-09-12 最终接管状态：本轮代码、独立审查修复与本机部署已完成；正式校准和部分历史索引继续由 worker 运行。验收与能力边界见 [本轮交付与验收](verification-2026-09-12.md)。以下日期段落保留历史实现说明；以最新交付为准。
...
2026-09-12 追加「锚定定位 + 校准分档」（迁移 0051、0052，按 `docs/redesign-2026-09-12.md` §5 实施）：这一轮改的是「给截图找行情」这件事的做法本身。从前是一路蛮力：把整张图的形状丢进向量检索，分不够就把窗口往更早推一段再试一遍。现在**先读图上写着的字，再去对形状**（§5.2 五步）：`chart_search/anchors.rs` 从 OCR 里认出时间轴刻度（`time_label` 容得下 `2026/09/07 18:00` 这样的整年标签和 `09-10 05:` 这样被裁掉半截的标签）、价格轴（一列数字必须**递减**，混进来的按最长递减子序列留下仍然成立的那几个，而不是整列丢掉）、极值标签和面板分界。时间轴给出周期与最后一根的时刻、价格轴给出这张图的价格带，两者合起来把「在几十万个窗口里找一段」缩成「在某个品种某个周期的某十根附近核对一下」。截图上的时刻是**本地时间**，所以要试的时区是 `[本人上次那张图的时区, 本机时区, UTC, +480]` 去重后逐个试，选中哪一段之后**回头**按最后一根最近的那个轴认时区（`locate_anchored.rs` 里的 `chosen_last` 那一步），并把它记进 `user_preferences.screenshot_utc_offset_minutes`，下次先试它。结果不再只有「定到了/没定到」两种：`outcome` 是 `located|candidates|needs_manual|already_located|unreadable`，`method` 是 `extremes|time_axis|shape_sweep|manual`，`anchors` 把读到的锚原样交出来（迁移 0051 的 `attachment_locations.anchor jsonb` 把它长期存下来，下次不用重读），门槛降到 `SCOREBOOK_AUTO_LOCATE_MIN_SCORE=0.75` / `SCOREBOOK_AUTO_LOCATE_MIN_MARGIN=0.10`，因为有了时间锚之后「分高但对错了地方」这件事已经由锚挡住了，再用 0.85 只是把对的也一起挡掉。**「往更早推一段再搜一遍」这条策略删掉了**：它解决的是没有锚时的瞎猜，有锚之后它只会把答案推离真值。手动那条路补成 `PUT /v1/attachments/{id}/location` 收 `{symbol, market, interval, end_at, bars_count?}`、当场回一段预览（真 K 线 + `match{score, level, reverse}`），人自己指的那一段也照样给分，不假装它一定对。

同日追加「一次重温放所有已定位的截图」与「稀有度分档」：重温从「一条记录一个品种」改成 `tracks[]`——这条记录下每一张定位过的截图各是一条轨，品种不同也一起给（`application/replay.rs`、`crates/http/src/replay_routes.rs`），主轨仍是场景截图那一条，其余轨照实标 `matched_by`。检索那一侧三件事一起动（§5.5）：窗口档位从 64/128/256 补成 **64/96/128/192/256**，查询只查离实际根数最近的两档（`history::nearest_windows`，五档全查是五倍 ANN 开销，差一档以上的窗口本来也排不到前面）；单品种那条 SQL 补上 `bars_count` 过滤，从前它不过滤，于是 32 根那种边角料窗口会混进来；精排改成**不定长**——在取回来的那段行情里按 `n ± 15%`、步长 4 扫一遍（`chart_search/rerank.rs::best_window` 调 `chart_match::sweep`），红绿两种假设各扫一遍取高者，`RERANK_BUDGET` 从 100 抬到 300。分数本身是个没有量纲的数，0.63 是高是低取决于这个周期这个根数上「随便两段行情」能撞出多少分，所以迁移 0052 建 `chart_match_calibration(interval, bars_bucket, sample_score)`，`chart.calibrate` 作业（体 `{interval, buckets?, samples?, candidates?, market?}`）拿库里随机窗口互相检索采样把这个分布量出来，检索结果里因此多了 `match.rarity`（这一档里有多少比例的样本比它低）、`match.level`（`sure|likely|weak`，按稀有度 0.97/0.85/0.6 分档，没有校准数据时退回按绝对分 0.75/0.6/0.45 分档，两套用同一组词免得同一张图在校准前后跳词）和整轮的 `verdict`（`found|weak|none`）。**后端只给枚举值，词是前端的事**：`sure/likely/weak/found/none` 这几个串是契约，不是文案。
