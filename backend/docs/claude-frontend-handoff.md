...
系统图用命中的 chart_request 调 `/v1/market/chart` 即时重绘。系统图（SVG）和匹配缩略图不入持久缓存；原始 K 线允许浏览器保存一份 24 小时到期的本机热缓存（IndexedDB，只存已收盘根，总量上限 50 MB，过期即清、用户可一键清空），口径与 `replay_bars` 的 24 小时临时缓存一致；仍然不做长期保存。这是用户 2026-09-13 的决定。不要把系统图冒充原始现场图。不同品种统一币安合约目录，不能推断任意美股 ticker 都有行情。
