...
/**
 * 一个字段。要说明的话收进标签旁边那颗问号里——填表的人需要它的时候摸得到，
 * 不需要的时候不挡着字段本身（改版 §8.6：操作说明移入 tooltip）。
 */
export function field(label: string, control: HTMLElement, hint?: string): HTMLElement {
  return h(
    ...
    h(
      ...
      {},
      h('span', { text: label }),
      hint ? h('span.qtip', { title: hint, attrs: { 'aria-label': hint } }, icon('q')) : null,
    ),
    control,
  )
}
...
/** 一节：一个标题和内容。默认收着，点开才展开。 */
export function panel(title: string, body: HTMLElement, open = false): HTMLElement {
