 ...
 */
export function unknown(_why?: string): HTMLElement {
  return h('span.faint', { text: '不知道' })
}
...
export function directionBadge(direction: 'long' | 'short' | string): HTMLElement {
  ...
  return h('span', { class: ['stance', long ? 'L' : 'S'], text: long ? '看多' : '看空' })
}
...
const SIDE_LABEL: Record<string, string> = { BUY: '看多', SELL: '看空' }
...
export function cycleStamp(status: string): HTMLElement {
  ...
  if (status === 'opening_unknown') return h('span.stamp.st-insufficient_data', { text: '不知道' })
