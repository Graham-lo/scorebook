...
import { money, unknown } from './fills-bits'
import {
  checkbox,
  field,
  inline,
...
import { exchangePanels } from './fills-exchange'
...
export function bookPanel(
  ...
  onChanged: () => void,
  onLedgerChanged: () => void = onChanged,
): HTMLElement {
  const host = h('div')
  host.appendChild(accountPanel(connection, all, onChanged))
  if (connection) {
    const imports = h('div', {}, importsPanel(connection))
    const refreshLedger = () => {
      onLedgerChanged()
      if (host.isConnected) imports.replaceChildren(importsPanel(connection))
    }
    host.append(seedPanel(connection, refreshLedger), csvPanel(connection, refreshLedger), reconcilePanel(connection))
    for (const node of exchangePanels(connection, refreshLedger)) host.appendChild(node)
...
function accountPanel(
  ...
  if (all.length) body.append(h('div.dlabel', { text: '已有账户' }), existing)
  ...
  const market = select(
    ...
    connection?.market ?? 'usd_m',
  )
  const keychain = input('scorebook.exchange.<你的账户>.<标签>')
  const create = h('button.btn.sm.primary', { text: '接账户' })
  create.addEventListener('click', () => {
    ...
    void submit(create, () => trades.connect(payload, connectAction.keyFor(payload)), () => {
      connectAction.reset()
      toast('记下了')
      onChanged()
    })
  ...
  body.append(
    ...
    h('div.dlabel', { text: '接一个账户' }),
    rows(
      inline(field('名字', name), field('账户标签', label), field('市场', market)),
      field(
        '钥匙串条目（可以先不填）',
        keychain,
        '留空也能导入 CSV；直连同步需填本机钥匙串条目名',
      ),
      inline(create),
    ),
  )

  if (!connection || !all.length) return panel('账户', body, true)
  ...
  const apply = h('button.btn.sm', { text: '确认更改' })
  ...
  body.append(
    h('div.hr', { style: 'margin:16px 0' }),
    h('div.dlabel', { text: '管理账户' }),
    rows(
      inline(field('账户', target), field('要做什么', action)),
      field('新的钥匙串条目', newKeychain, '断开时不用填'),
      inline(apply),
    ),
  )

  return panel('账户', body, all.length === 0)
}

// ——— 期初持仓 ———

function seedPanel(connection: ExchangeConnection, onChanged: () => void): HTMLElement {
  ...
  const evidence = textarea('从哪儿看到的')
...
function csvPanel(connection: ExchangeConnection, onChanged: () => void): HTMLElement {
  ...
  file.addEventListener('change', () => {
    ...
    void picked.text().then((content) => {
      if (file.files?.[0] === picked) text.value = content
    }).catch(() => {
      if (file.files?.[0] === picked) problem('文件没读出来，请重新选择')
    })
  })
  ...
  save.addEventListener('click', () => {
    ...
    void submit(save, () => trades.importCsv(payload, csvAction.keyFor(payload)), (result) => {
      csvAction.reset()
      onChanged()
      toast(`记下了 · 新增 ${result.inserted_fills} 笔 · 重复 ${result.duplicate_fills} 笔`)
    })
  })
...
function reconciliationView(outcome: ReconciliationResult): HTMLElement {
  ...
  if (outcome.status === 'unverified_coverage') {
    ...
    host.appendChild(note('info', '每个币种都对上了'))
  } else {
    host.appendChild(note('warn', '有对不上的地方'))
  }

  if (outcome.assets_missing_from_statement.length) {
    host.appendChild(
      note('warn', `对账单上没填：${outcome.assets_missing_from_statement.join('、')}`),
    )
  }
    ...
    host.appendChild(
      h(
        ...
        asset.fills_missing_realized_pnl > 0
          ? h('div.tip', {
              style: 'margin-top:4px',
              text: `${asset.fills_missing_realized_pnl} 笔成交没有已实现盈亏`,
            })
          : null,
        lines,
      ),
  ...
  void (async () => {
    try {
      ...
      if (!page.items.length) {
        list.appendChild(empty({ title: '还没有导入过' }))
        return
      }
