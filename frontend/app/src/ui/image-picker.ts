...
import { attachmentImage, objectUrl } from './media'
...
export function imagePicker(model: ImageUploads, label: string, changed: () => void, enabled: () => boolean = () => true) {
  ...
  const node = h('section.image-picker', {}, h('div.row', { style: 'gap:10px;flex-wrap:wrap' }, add, count), h('div.tip', { text: `可以一次选多张，也可以拖入或粘贴；最多 ${model.limit} 张。` }), input, grid)
  ...
  function render(): void {
    ...
    count.textContent = model.items.length ? `${model.items.length} 张${model.pending ? ' · 尚未全部上传' : ' · 已上传'}` : '可选'
    ...
    for (const [i, item] of model.items.entries()) {
      ...
      if (!card) {
        const shot = item.preview
          ...
          : item.id ? attachmentImage(item.id, { alt: `截图 ${i + 1}`, maxWidth: 420 }) : h('span')
        const status = h('span')
      ...
      const status = item.error ?? (item.uploading ? `上传中 ${Math.round(item.progress * 100)}%` : `截图 ${i + 1}`)
      ...
      if (item.preview) card.shot.setAttribute('aria-label', `查看截图 ${i + 1}`)
...
export function reviewImages(ids: string[] = [], label = '复盘时的后续走势'): HTMLElement | null {
  ...
  return h('div.review-shots', {}, h('div.dlabel', { text: label }),
    h('div.upload-images', {}, ...ids.map((id, i) => attachmentImage(id, {
      alt: `${label} ${i + 1}`,
      // 列表里放缩略图，点开才去取全尺寸那张：一屏几张后续走势图，全按原尺寸解码
      // 手机会烫。图一张都没少，只是等到你要看的时候才下载。
      maxWidth: 560,
      onReady: (_url, image) => {
        image.style.cursor = 'zoom-in'
        image.setAttribute('role', 'button')
        image.setAttribute('tabindex', '0')
        image.setAttribute('aria-label', `放大查看${label} ${i + 1}`)
        const open = () => void objectUrl(id).then((full) => lightbox(full, `${label} ${i + 1}`)).catch(() => problem('这张图读不出来。'))
