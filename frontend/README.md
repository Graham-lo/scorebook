...
- 币安全历史的覆盖不是全量：能搜到的只有已经发布的那些范围，目录里有不等于能搜。
  按用户的要求暂不启动历史同步，所以现在的公开历史覆盖是空的——这是预期状态。
- 正式统计的口径已经有了，但那是「特定规则和历史范围下的参照」，不是策略预测，更不
  是胜率。

## 2026-09-10 周期与历史测试更新

截图检索现在必须先选定周期，只比较同周期。历史准备支持读取已核实上线时间至最新收盘的范围，选范围不自动下载。按用户要求暂不启动历史同步。真实小范围测试已通过，测试库/临时目录及主库演示公共历史索引已经清理；当前公开历史覆盖为空是预期状态。用户记录、原图和复盘保留。详见后端 docs/period-live-verification.json、docs/public-history-cleanup.json 及桌面 Scorebook_Claude前端重构Prompt_v4.1.md。
