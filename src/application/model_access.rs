...
pub async fn call(s: &Services, owner: Uuid, input: ToolCall) -> Result<Value> {
    let result = match input.name.as_str() {
        ...
        "render_market_chart" => {
            json!({"mime":"image/svg+xml","svg":super::market::svg(&serde_json::from_value(input.arguments).map_err(|_|Error::bad("invalid_tool_arguments"))?).await?,"storage":"not_persisted"})
        }
        "search_knowledge" => {
        ...
        "search_similar_charts" => {
            let query = serde_json::from_value(input.arguments)
                .map_err(|_| Error::bad("invalid_tool_arguments"))?;
            similarity::search(s, owner, &format!("tool-{}", Uuid::new_v4()), query).await?
        }
        "read_attachment" => {
            let id = id(&input.arguments)?;
