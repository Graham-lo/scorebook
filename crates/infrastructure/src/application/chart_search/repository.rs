...
pub async fn public_candidates(
    ...
    let rows=sqlx::query("WITH candidates AS MATERIALIZED (SELECT id,market,symbol,timeframe,start_at,end_at,bars_count,input_hash,embedding::vector(192) <=> $1::vector(192) AS distance FROM public_market.features WHERE model_id='candle-geometry-v2' AND published AND end_at<=$2 AND ($3::text IS NULL OR symbol=$3) AND ($4::text IS NULL OR market=$4) AND timeframe=$5 AND bars_count=ANY($6) ORDER BY embedding::vector(192) <=> $1::vector(192) LIMIT 3000) SELECT * FROM candidates ORDER BY distance+0,id LIMIT 1000")
...
pub async fn private_candidates(
    ...
    for (model, dimension, vector) in [
        ...
        let sql = format!(
          ...
          AND EXISTS(SELECT 1 FROM call_attachments l JOIN calls c ON c.owner_id=l.owner_id AND c.id=l.call_id JOIN attachments a ON a.owner_id=l.owner_id AND a.id=l.attachment_id WHERE l.owner_id=e.owner_id AND l.attachment_id=e.attachment_id AND c.submitted_at<=$4 AND a.kind='scene' AND a.uploaded_at<=c.submitted_at AND (a.captured_at IS NULL OR a.captured_at<=c.submitted_at) AND ($5::text IS NULL OR c.instrument=$5) AND ($6::text IS NULL OR c.market=$6) AND c.timeframe=$7 OFFSET 0)
          ...
          WHERE c.submitted_at<=$4 AND a.uploaded_at<=c.submitted_at AND (a.captured_at IS NULL OR a.captured_at<=c.submitted_at) AND ($5::text IS NULL OR c.instrument=$5) AND ($6::text IS NULL OR c.market=$6) AND c.timeframe=$7
