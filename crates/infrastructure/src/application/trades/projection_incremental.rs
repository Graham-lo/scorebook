//! Constant-size book snapshots share closed-cycle epochs. Only new fills and
//! the open cycle change; historical inserts rebuild just their affected book.
use super::*;
...
pub async fn prepare(
    ...
    let row=sqlx::query("SELECT p.run_id,p.seed_hash,p.last_at,p.last_sequence::text,p.body,r.ledger_revision,b.epoch_id,b.open_cycle_id FROM trade_projection_heads h JOIN trade_projection_runs r ON r.id=h.run_id JOIN trade_projection_checkpoints p ON p.owner_id=h.owner_id AND p.run_id=h.run_id JOIN trade_book_snapshots b ON b.owner_id=p.owner_id AND b.run_id=p.run_id AND b.symbol=p.symbol AND b.position_side=p.position_side WHERE h.owner_id=$1 AND h.connection_id=$2 AND p.symbol=$3 AND p.position_side=$4").bind(j.owner).bind(connection).bind(&seed.symbol).bind(side(&seed.position_side)).fetch_optional(&s.db.pool).await?;
    ...
    if let Some(row) = row {
        ...
        if row.get::<String, _>("seed_hash") == digest(&json!(seed)) && !historical {
            ...
            if !changed {
                ...
                sqlx::query("INSERT INTO trade_book_snapshots SELECT owner_id,$3,symbol,position_side,epoch_id,closed_through,open_cycle_id FROM trade_book_snapshots WHERE owner_id=$1 AND run_id=$2 AND symbol=$4 AND position_side=$5").bind(j.owner).bind(old).bind(run).bind(&seed.symbol).bind(side(&seed.position_side)).execute(&mut *tx).await?;
            ...
            if projector.current(None).fills > 0 {
                let old_cycle: Option<Uuid> = row.get("open_cycle_id");
                let old_cycle =
...
pub async fn finish(
    ...
    let current = state.projector.current(None);
    let open = if current.fills > 0 {
        Some(super::projection::cycle_id(
            run,
            &seed.symbol,
            side(&seed.position_side),
            current.ordinal,
    ...
    sqlx::query("INSERT INTO trade_book_snapshots(owner_id,run_id,symbol,position_side,epoch_id,closed_through,open_cycle_id) VALUES($1,$2,$3,$4,$5,$6,$7)").bind(j.owner).bind(run).bind(&seed.symbol).bind(side(&seed.position_side)).bind(state.epoch_id).bind(current.ordinal as i64-1).bind(open).execute(&mut *tx).await?;
    tx.commit().await?;
    Ok(())
}
