...
pub async fn replay(
    ...
    sqlx::query("SELECT id FROM calls WHERE owner_id=$1 AND id=$2 FOR UPDATE")
        ...
        .fetch_one(&mut *tx)
        .await?;
    let existing:bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM outcomes WHERE owner_id=$1 AND call_id=$2 AND claim_no=0 AND kind='rule_replay' AND digest=$3)").bind(owner).bind(id).bind(&business_digest).fetch_one(&mut *tx).await?;
    if !existing {
        sqlx::query(
            "INSERT INTO manifests(id,owner_id,call_id,digest,body) VALUES($1,$2,$3,$4,$5)",
        )
        .bind(mid)
        .bind(owner)
        .bind(id)
        ...
        .execute(&mut *tx)
        .await?;
        sqlx::query("INSERT INTO outcomes(id,owner_id,call_id,claim_no,manifest_id,kind,result,digest) VALUES($1,$2,$3,0,$4,'rule_replay',$5,$6)").bind(oid).bind(owner).bind(id).bind(mid).bind(json!(result)).bind(&business_digest).execute(&mut *tx).await?;
    }
