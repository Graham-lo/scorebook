...
pub async fn build(s: &Services, j: &Job) -> Result<Value> {
    ...
    for batch in chunks.chunks(4) {
        let output = s
            ...
            .await?;
        if output.model_id != MODEL || output.weights_sha256 != WEIGHTS {
            return Err(Error::bad("text_encoder_identity_mismatch"));
        }
    ...
    .execute(&mut *tx)
    .await?;
    sqlx::query("INSERT INTO knowledge_documents(id,owner_id,source_kind,source_id,source_version,occurred_at,content,source_uri,indexed_revision) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)").bind(doc).bind(j.owner).bind(&kind).bind(id).bind(&version).bind(occurred_at).bind(&text).bind(format!("scorebook://knowledge/{kind}/{id}?version={version}")).bind(revision).execute(&mut *tx).await?;
    let rows:Vec<Value>=chunks.iter().zip(embeddings).enumerate().map(|(ordinal,((start,end,text),embedding))|json!({"id":Uuid::new_v4(),"ordinal":ordinal,"start_byte":start,"end_byte":end,"content":text,"content_sha256":hash_bytes(text.as_bytes()),"embedding":format!("[{}]",embedding.iter().map(f32::to_string).collect::<Vec<_>>().join(","))})).collect();
    for batch in rows.chunks(500) {
        sqlx::query("INSERT INTO knowledge_chunks(id,owner_id,document_id,ordinal,start_byte,end_byte,content,content_sha256) SELECT r.id,$1,$2,r.ordinal,r.start_byte,r.end_byte,r.content,r.content_sha256 FROM jsonb_to_recordset($3) r(id uuid,ordinal int,start_byte int,end_byte int,content text,content_sha256 text)").bind(j.owner).bind(doc).bind(json!(batch)).execute(&mut *tx).await?;
        sqlx::query("INSERT INTO knowledge_embeddings(owner_id,chunk_id,model_id,weights_sha256,embedding) SELECT $1,r.id,$3,$4,r.embedding::vector FROM jsonb_to_recordset($2) r(id uuid,embedding text)").bind(j.owner).bind(json!(batch)).bind(MODEL).bind(WEIGHTS).execute(&mut *tx).await?;
    }
    sqlx::query("DELETE FROM knowledge_dirty WHERE owner_id=$1 AND source_kind=$2 AND source_id=$3 AND revision=$4").bind(j.owner).bind(kind).bind(id).bind(revision).execute(&mut *tx).await?;
